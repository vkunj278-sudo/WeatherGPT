...
from backend.condition_service import analyze_conditions, analyze_forecast_conditions
...
from backend.alert_manager import create_alert, get_alerts, clear_alerts, resolve_alert
...
@app.get("/health")
def health():
    from backend.ai_service import API_KEY, DEEPSEEK_API_KEY, MODEL_NAME, DEEPSEEK_MODEL
    return {
        "status": "healthy",
...
def forecast(city: str):
    ...
    return get_forecast(location_data["latitude"], location_data["longitude"])
...
def ask_ai_test(question: str):
    return {"question": question, "answer": ask_ai(question)}


@app.get("/alerts")
def alerts(city: str = None, status: str = None):
    return {"alerts": get_alerts(city=city, status=status)}


@app.get("/alerts/{city}")
def city_alerts(city: str):
    return {"city": city, "alerts": get_alerts(city=city)}
...
def smart_weather(
    ...
    # CRITICAL: pass selected UI location separately.
    # The selected city must never be appended to the user's text.
    ...
    city = understanding.get("city", "UNKNOWN")
    intent = understanding.get("intent", "GENERAL_WEATHER")
    time_period = understanding.get("time", "UNKNOWN")
    ...
    if data_source == "current":
        ...
        forecast_data = filter_forecast(forecast_data, time_period)
    ...
    if weather_data and not forecast_data:
        result = analyze_conditions(weather_data)
        conditions = result.get("conditions", [])
        severity = determine_severity(conditions)
        intelligence = {
            "conditions": conditions,
            "severity": severity,
            "recommendations": generate_recommendations(conditions),
            "warnings": generate_warnings(conditions, severity),
        }

    elif forecast_data and not weather_data:
        result = analyze_forecast_conditions(forecast_data)
        conditions = result.get("conditions", [])
        severity = determine_severity(conditions)
        intelligence = {
            "conditions": conditions,
        ...
        current_result = analyze_conditions(weather_data)
        forecast_result = analyze_forecast_conditions(forecast_data)

        current_conditions = current_result.get("conditions", [])
        forecast_conditions = forecast_result.get("conditions", [])

        conditions = list(dict.fromkeys(current_conditions + forecast_conditions))
        severity = determine_severity(conditions)
